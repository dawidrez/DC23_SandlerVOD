...
def send_custom_emails():
    ...
    for subscription in expiring_subscriptions:
        ...
        if client.gender == GenderTypes.FEMALE:
            message = f'Cześć {client.first_name},\n\nZauważyliśmy, że Twoja subskrypcja na pakiet "{package.name}" kończy się {subscription.end_date}. Czy chciałabyś ją przedłużyć?\n\nPozdrawiamy,\nZespół SandlerVOD'
        else:
...
#
    ...
    for client in Client.objects.all():
        ...
        if new_package:
            ...
            if client.gender == GenderTypes.FEMALE:
                message = f'Cześć {client.first_name},\n\nZastanawiałaś się nad wypróbowaniem naszego pakietu "{new_package.name}"? Zawiera on wiele interesujących filmów, które mogą Cię zainteresować.\n\nPozdrawiamy,\nZespół SandlerVOD'
            else:
